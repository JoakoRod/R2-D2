...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!channel) return message.reply("Entra a un canal de voz gord@ fracasad@").catch(console.error);
    ...
    if (urlValid) {
      ...
      try {
        ...
        if (error.message.includes("410")) {
          return message.reply("Video Prohibido pa, que guarang@").catch(console.error);
        } else {
          return message.reply(error.message).catch(console.error);
        }
      }
